...
# Global variable to hold the current angle from the angle detection thread.
current_angle = None

def load_class_names(csv_path):
    """
    Load YAMNet class names from a CSV file.
    Assumes CSV rows are: index, mid, display_name.
    """
    class_names = []
    skipped = False
...
def angle_thread(stop_event):
    """
    Continuously reads from the USB device to determine the sound direction.
    Updates the global variable 'current_angle'.
    """
...
def send_message(host: str, port: int, message: str):
    """
    Connects to the specified host/port and sends the message.
    Prints the connection status and any errors encountered.
    """
    try:
...
def sound_classification_thread(stop_event, device_id, metaquest_host, metaquest_port):
    """
    Records audio from the specified device, computes its volume (in dB),
    runs inference with the YAMNet model, and sends a message containing the
    current sound direction, audio magnitude, and top prediction to MetaQuest.
    """
    ...
    try:
        with sd.InputStream(device=device_id, channels=6, samplerate=sample_rate) as stream:
            while not stop_event.is_set():
                ...
                # Select channel 0 for processing.
                ...
                # Convert waveform to tensor and run YAMNet inference.
                ...
                # Use the current angle from the angle thread (if available).
...
def main():
    """
    Lists available sound devices, parses command-line arguments,
    starts the angle detection and audio classification threads,
    and keeps the main thread running until a KeyboardInterrupt.
    """
    ...
    parser = argparse.ArgumentParser(
        description="Sound detection and UDP messaging to MetaQuest."
    )
    parser.add_argument('--device', type=int, default=None,
                        help="Sound device ID to use for recording.")
    parser.add_argument('--metaquest-host', type=str, default="127.0.0.1",
                        help="MetaQuest host IP address.")
    parser.add_argument('--metaquest-port', type=int, default=7000,
    ...
    # Start the angle detection and audio classification threads.
